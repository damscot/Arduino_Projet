...
#define RH433_TX_PIN 7
#define RH433_RX_PIN 9
...
#define LEFT_VRX_APIN  0
#define LEFT_VRY_APIN  1
#define RIGHT_VRX_APIN 6
#define RIGHT_VRY_APIN 7
...
LiquidCrystal lcd(SW1_PIN);

#ifdef RH33
RH_ASK RH_433 = RH_ASK (2000, RH433_RX_PIN, RH433_TX_PIN, -1, false);
#endif
...
void on_item1_selected(MenuItem* p_menu_item) {
  char str[16];
  lcd.setCursor(0,1);
  snprintf(str,15,"Starting %03d",ConfValue);
  lcd.print(str);
...
void gyro_loop(float *ret_angle_x, float *ret_angle_y, float *ret_angle_z)
{
  int error;
  double dT;
  accel_t_gyro_union accel_t_gyro;
...
void setup() {
  ...
  pinMode(ENCFORW_PIN, INPUT);
  pinMode(ENCBACKW_PIN, INPUT);
  pinMode(SW0_PIN, OUTPUT);
  pinMode(SW1_PIN, OUTPUT);
  digitalWrite(ENCSW_PIN, HIGH);
  digitalWrite(ENCFORW_PIN, HIGH);
  digitalWrite(ENCBACKW_PIN, HIGH);
  digitalWrite(SW0_PIN, HIGH);
  digitalWrite(SW1_PIN, HIGH);
  
  pinMode(LEFT_SW_APIN, INPUT);
  pinMode(RIGHT_SW_APIN, INPUT);
  digitalWrite(LEFT_SW_APIN, HIGH);
  digitalWrite(RIGHT_SW_APIN, HIGH);
...
void voiture(int modeset) {
 ...
 // LEFT Stick
 left_posx = map(analogRead(LEFT_VRX_APIN), 0, 1023, 0, 99);
 left_posy = map(analogRead(LEFT_VRY_APIN), 0, 1023, 0, 99);
 // RIGHT Stick
 right_posx = map(analogRead(RIGHT_VRX_APIN), 0, 1023, 0, 99);
 right_posy = map(analogRead(RIGHT_VRY_APIN), 0, 1023, 0, 99);
 
 gyro_loop(&angle_x,&angle_y,&angle_z);
  ...
  if (mode == 0) {
    ...
    x = map(angle_x, -25, 25, 0, 99);
    y = map(angle_y, -25, 25, 0, 99);
    Serial.println(x);
    Serial.println(y);
    buf[2] = '0' + (x/10);
    buf[3] = '0' + (x%10);
  ...
  //sprintf(str,"Val:%d",newPosition);
  //sprintf(str,"sw0:%d",sw0);
  //sprintf(str,"sw1:%d",sw1);
  //sprintf(str, "SW:%d,%d,%d",encsw,sw0,sw1);
  sprintf(str, "L:%02d,%02d  R:%02d,%02d",left_posx,left_posy,right_posx,right_posy);
  //sprintf(str, "%d,%d%",left_sw,right_sw);
  //Serial.println(str);

  lcd.setCursor(0,0);
  lcd.print(str);
  
  sprintf(str," X%c%02d Y%c%02d Z%c%02d",
    (((int)(angle_x) > 0) ? '+' :'-'), 
    abs((int)(angle_x)),
    (((int)(angle_y) > 0) ? '+' :'-'), 
    abs((int)(angle_y)),
